...
import com.admin.Addash;
import com.toedter.calendar.JDateChooser;

public class Stupdate implements ActionListener{
	JFrame screen;
	...
	JTextField first,middle,last,Area,Nation,Username,Email,Smbno,Father,Foccup,Faddress,Fmbno,Mother,Moccup,Mmbno,Maddress,id;
	...
	static final String DB_URL = "jdbc:mysql://localhost:3306/project";
	 ...
	 Date sm;
	 String f,l,m,pop,mum,sd,smobile,fmobile,mmobile,Foccupp,Moccupp,Maddresss,Faddresss,almuni,session,mail,user,pas,reg,gend,branchh,statess,Areaa,Nationn,pic,regis;
	public Stupdate() throws Exception{
...
for (int j= 0 ; j< 4; j++) { comboboxyear.addItem(YEar[j]); }
	...
	public void actionPerformed(ActionEvent e){
	  ...
	  if(e.getSource()==get){
			...
			try{if(regis.equals(reg)&& (!almuni.equals("student"))){JOptionPane.showMessageDialog(screen,"INVALID CREDENTIAlS");}
			else if(reg.equals("")){JOptionPane.showMessageDialog(screen,"PLEASE ENTER REGISTRATION NUMBER");}
				else if(regis.equals(reg)&& almuni.equals("student")){
				 												pi.setVisible(true);
...
combobox1.setVisible(true);
			 ...
			 comboboxyear.setSelectedItem(session);
			   Email.setText(mail);
			   Username.setText(user);
			  Password.setText(pas);

              comboboxbranch.setSelectedItem(branchh);
			   combobox1.setSelectedItem(statess);
			   Area.setText(Areaa);
        ...
        gend="FEMALE";
			...
			try{System.out.println("Creating statement...");
    		  stmt = conn.createStatement();
			  String sql;
			  sql="UPDATE  member SET firstname='"+f+"',middlename='"+m+"',lastname='"+l+"',fathername='"+pop+"',mothername='"+mum+"',gender='"+gend+"',branch='"+branchh+"',dob='"+sd+"',mobileno='"+smobile+"',year='"+session+"',emailid='"+mail+"',username='"+user+"',password='"+pas+"',almuni='student',foccupation='"+Foccupp+"',fmbno='"+fmobile+"',faddress='"+Faddresss+"',moccupation='"+Moccupp+"',mmbno='"+mmobile+"',maddress='"+Maddresss+"',area='"+Areaa+"',nationality='"+Nationn+"',state='"+statess+"'WHERE regno='"+reg+"'";
			  ...
			  System.out.println(f+l+m+pop+mum+sd+smobile+fmobile+mmobile+Foccupp+Moccupp+Maddresss+Faddresss+session+mail+user+pas+reg+gend+branchh+statess+Areaa+Nationn);
		        JOptionPane.showMessageDialog(screen,"STUDENT UPDATED SUCCESSFULLY");
